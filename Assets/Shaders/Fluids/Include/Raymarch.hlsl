...
#ifndef VOLUMETRIC_INCLUDE
#define VOLUMETRIC_INCLUDE
...
void RaymarchFluid_float(float3 rayOrigin, float3 rayDirection, float numSteps, float stepSize,
                         float densityScale, UnityTexture3D densityTex, UnitySamplerState volumeSampler,
                         ...
                         float lightAbsorb, float darknessThreshold, float transmittance, out float3 result)
{
    ...
    for (int i = 0; i < numSteps; i++)
    {
        ...
        //     float lightDensity = SAMPLE_TEXTURE3D(densityTex, volumeSampler, lightRayOrigin).r;
